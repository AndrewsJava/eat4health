...
public class HistogramView extends FrameLayout {
	...
	public HistogramView(Context context, StatTool blDn, int sw, int sh, int bgcolor) {
		...
		this.nutrientPt = Eat4Health.db[SubScroll.currentNutrientID][SubScroll.currentFoodID];

		boolean useServingOK = true;

		switch (Eat4Health.Nutrient_Measure) {
		case Eat4Health.USING_KCAL:
			this.nutrientPt = Eat4Health.getPer100KcalDataPoint(SubScroll.currentFoodID, this.nutrientPt);

			break;
		case Eat4Health.USING_SERVING:
			this.nutrientPt = Eat4Health.getPerServingDataPoint(SubScroll.currentFoodID, this.nutrientPt);
		...
		this.title = "Distribution of " + Eat4Health.nutrients[SubScroll.currentNutrientID] + " in all foods in: "
				+ Eat4Health.getFoodsSearchDescription();

		this.title2 = "[" + Eat4Health.foods[SubScroll.currentFoodID] + "]" + " contains ";
		this.title3 = this.nutrientPt + " " + Eat4Health.units[SubScroll.currentNutrientID] + " of ["
				+ Eat4Health.nutrients[SubScroll.currentNutrientID] + "]";
		...
		case Eat4Health.USING_SERVING:
			...
			if (useServingOK) {
				this.title3 += " in "
						...
						+ Eat4Health.oddUnits[SubScroll.currentFoodID][Eat4Health.optimalServingId[SubScroll.currentFoodID]] + " of "
						+ Eat4Health.foods[SubScroll.currentFoodID];
			} else {
	...
	protected void onDraw(Canvas c) {
		...
		c.drawText(" " + Double.toString((double) ((int) (10000.0 * (upperLim))) / 10000.0), frameW - 120, frameH - 20, _paints[3]);
		invalidate();
	}

...
public class PreferencesScroller extends SubScroll {
	...
	CompoundButton.OnCheckedChangeListener foodGroupPreferencesListener = new CompoundButton.OnCheckedChangeListener() {
		...
		public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
			...
			Eat4Health.MY_FOOD_GROUPS[buttonView.getId()] = isChecked;
			Eat4Health.saveObject(Eat4Health.MY_FOOD_GROUPS, "MYFOODGROUPS");
			if (Eat4Health.USE_MY_FOOD_GROUPS == Eat4Health.Foods_Search)
	...
	CompoundButton.OnCheckedChangeListener nutrientsPreferencesListener = new CompoundButton.OnCheckedChangeListener() {
		...
		public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
			...
			if (isChecked) {
				Eat4Health.nutritionStats.put(bID, new StatTool(Eat4Health.getFoodSearchIds(), bID));
			}
			Eat4Health.MY_NUTRIENTS[bID] = isChecked;
			int[] foodIds = Eat4Health.getFoodSearchIds();
			float[] basically = StatTool.simpleStats(foodIds, bID);

			Eat4Health.highlightFactors.put(bID, basically);

			Eat4Health.saveObject(Eat4Health.MY_NUTRIENTS, "MYNUTRIENTS");
	...
	PreferencesScroller(Context c, int type) {
		...
		if (type == 0) {
			...
			addScrollingButtons(Eat4Health.foodGroups, ids, Eat4Health.MY_FOOD_GROUPS, foodGroupListener, FOOD_GROUP_LABEL_COLOR, type);
		...
		if (type == 1) {
			...
			addScrollingButtons(Eat4Health.nutrients, ids, Eat4Health.MY_NUTRIENTS, foodGroupListener, BUTTON_COLORS[GROUPPREFS], type);
	...
	public void addScrollingButtons(String[] buttonNames, int[] buttonIds, boolean[] settings, View.OnClickListener listener,
			int BUTTON_CLR, int type) {
		...
		for (int i = 0; i < buttonNames.length; i++) {
			...
			if (type == 0) {
				...
				buttons[i].getBackground().setColorFilter(colorCombo, PorterDuff.Mode.SRC_IN);

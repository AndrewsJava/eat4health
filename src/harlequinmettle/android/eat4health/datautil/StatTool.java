...
public class StatTool {
	...
	public void doStatsOnList(int[] foodIds, int nutrientID) {
		...
		outer: for (int i : foodIds) {
			float dataPoint = Eat4HealthData.db[nutrientID][i];

			if (dataPoint < 0)
				continue;
			if (Eat4HealthData.USING_NOT_ZEROS == Eat4HealthData.DATA_CHOICES && dataPoint == 0)
				continue outer;

			switch (Eat4HealthData.Nutrient_Measure) {
			case Eat4HealthData.USING_GRAMS:
				// DONT REALLY NEED THIS - ALREADY USING PER 100G BY DEFAULT
				break;
			case Eat4HealthData.USING_KCAL:
				dataPoint = Eat4HealthLegacy.getPer100KcalDataPoint(i, dataPoint);

				break;

			case Eat4HealthData.USING_SERVING:

				dataPoint = Eat4HealthLegacy.getPerServingDataPoint(i, dataPoint);
				if (dataPoint < 0)
					dataPoint = Eat4HealthData.db[nutrientID][i];
